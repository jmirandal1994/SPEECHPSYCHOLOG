...
export default function Workers() {
  ...
  async function loadData() {
    ...
    // Try to load projects — fallback to all if active filter fails
    let projData = []
    const { data: p1, error: pe1 } = await supabase
      .from('projects').select('id, name, active').eq('active', true).order('name')
    if (!pe1 && p1) {
      projData = p1
    } else {
      // Fallback: get all projects regardless of active
      const { data: p2 } = await supabase
        .from('projects').select('id, name, active').order('name')
      projData = p2 || []
    }
  ...
  const filtered = workers.filter(w =>
    [(w.full_name||''),(w.project||''),(w.role_label||'')].join(' ').toLowerCase().includes(search.toLowerCase())
  )
      ...
      <div className="content">
        ...
        <div className="card">
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Profesional</th><th>Cargo</th><th>Proyecto asignado</th><th>Contacto</th><th>Estado</th><th>Acciones</th></tr>
              </thead>
              <tbody>
                {loading ? (
                    ...
                    <td>
                      {projects.length > 0 ? (
                        ...
                        <input
                          ...
                          style={{marginBottom:0,fontSize:12,padding:'5px 8px',maxWidth:180}}
                          placeholder="Escribir proyecto..."
                          defaultValue={w.project||''}
                          onBlur={e=>{ if(e.target.value!==w.project) quickProject(w.id,e.target.value) }}
                        />
                      )}
                    </td>
