...
const PAY_STATUS = {
  pending:    { cls: 'badge-amber', label: '⏳ Pendiente de pago',  icon: '⏳', color: '#d97706' },
  in_process: { cls: 'badge-blue',  label: '🔄 En proceso',         icon: '🔄', color: '#1d4ed8' },
  paid:       { cls: 'badge-green', label: '✅ Pago recibido',       icon: '✅', color: '#059669' },
}

// Full amount formatter: 250000 → "$250.000 CLP"
...
export default function WorkerBoletas() {
  ...
  async function loadData() {
    ...
    const [{ data: pay }, { data: sh }] = await Promise.all([
      supabase.from('payments').select('*').eq('worker_id', user.id)
        .order('period_year', { ascending: false })
        .order('period_month', { ascending: false }),
      supabase.from('shifts').select('*').eq('worker_id', user.id)
  ...
  const thisMonth    = earningsByMonth[thisKey] || { shifts: [], total: 0 }
      ...
      <div className="content">
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:16 }}>
          ...
          <div className="stat-card">
            ...
            <div style={{ fontFamily:'var(--font-display)',fontWeight:800,fontSize:18,color:'var(--text-1)',lineHeight:1.1,marginTop:6,letterSpacing:'-0.02em' }}>
              ${thisMonth.total.toLocaleString('es-CL')}
            </div>
            <div style={{ fontFamily:'var(--font-body)',fontSize:11,color:'var(--text-4)',marginTop:4 }}>
              CLP · {thisMonth.shifts.length} turno{thisMonth.shifts.length!==1?'s':''}
            </div>
          </div>
          ...
          <div className="stat-card">
            ...
            <div style={{ fontFamily:'var(--font-body)',fontSize:11,color:totalPending>0?'var(--amber)':'var(--text-4)',marginTop:4,fontWeight:totalPending>0?700:400 }}>
        ...
        {tab === 'resumen' && (
          <>
            <div className="card" style={{ marginBottom:14 }}>
              ...
              {/* Big amount display */}
              <div style={{ background:'linear-gradient(135deg,var(--navy-800),var(--navy-900))',borderRadius:'var(--r-md)',padding:'20px 22px',marginBottom:16,display:'flex',alignItems:'center',justifyContent:'space-between' }}>
                <div>
                  ...
                  <div style={{ fontFamily:'var(--font-display)',fontSize:28,fontWeight:800,color:'#22c55e',letterSpacing:'-0.04em' }}>
                    ${thisMonth.total.toLocaleString('es-CL')}
                  </div>
              ...
              {thisMonth.shifts.length === 0 ? (
                ...
                <div key={s.id} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:'1px solid var(--border)' }}>
                  <div>
                    <div style={{ fontWeight:600,fontSize:13,color:'var(--text-1)' }}>{s.project || '—'}</div>
                    <div style={{ fontSize:11,color:'var(--text-4)',fontFamily:'var(--font-mono)',marginTop:2 }}>
                      {s.shift_date} · {s.start_time} – {s.end_time}
                    </div>
                  </div>
                  <div style={{ fontFamily:'var(--font-mono)',fontWeight:800,fontSize:14,color:'var(--emerald)',flexShrink:0 }}>
                    {s.fee ? fmtCLP(s.fee) : '—'}
                  </div>
                </div>
              ))}
            ...
            <div className="card">
              ...
              {Object.entries(earningsByMonth).length === 0 ? (
                ...
                <div key={p.id} className="card" style={{ marginBottom:12,padding:0,overflow:'hidden' }}>
                  ...
                  <div style={{ padding:'18px 20px' }}>
                    <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:14 }}>
                      <div>
                        <div style={{ fontFamily:'var(--font-display)',fontSize:16,fontWeight:700,color:'var(--text-1)',marginBottom:4 }}>
                    ...
                    {/* BIG amount — full format */}
                    <div style={{
                      background:p.status==='paid'?'var(--emerald-l)':p.status==='in_process'?'var(--navy-100)':'var(--amber-l)',
                      borderRadius:'var(--r)',padding:'16px 18px',
                      display:'flex',alignItems:'center',justifyContent:'space-between',
                    }}>
                      <div>
                        <div style={{ fontSize:11,fontWeight:700,color:p.status==='paid'?'var(--emerald-d)':p.status==='in_process'?'var(--navy-700)':'#92400e',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:4 }}>
                          Monto del pago
                        </div>
                        <div style={{
                          ...
                          fontSize:26,
                          fontWeight:800,
                          color:p.status==='paid'?'var(--emerald-d)':p.status==='in_process'?'var(--navy-700)':'#92400e',
                          letterSpacing:'-0.04em',
                        }}>
                          ${p.amount.toLocaleString('es-CL')}
                        </div>
                        <div style={{ fontSize:13,fontWeight:600,color:p.status==='paid'?'var(--emerald-d)':p.status==='in_process'?'var(--navy-700)':'#92400e',marginTop:2 }}>
                          CLP
                        </div>
                      </div>
                      <div style={{ fontSize:36 }}>
                        {p.status==='paid'?'✅':p.status==='in_process'?'🔄':'⏳'}
                      </div>
                    </div>

                    {p.paid_at && (
                      <div style={{ fontSize:11,color:'var(--text-4)',marginTop:10,fontFamily:'var(--font-mono)' }}>
                        Pagado el: {new Date(p.paid_at).toLocaleDateString('es-CL',{day:'2-digit',month:'long',year:'numeric'})}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="card">
                  ...
                  <tbody>
                    {shifts.map(s => (
                      <tr key={s.id}>
                        ...
                        <td><span className={`badge ${s.status==='completed'?'badge-green':'badge-amber'}`}>
                          {s.status==='completed'?'✓ Completado':'⚠ Con atraso'}
                        </span></td>
                        <td>
                          <span style={{ fontFamily:'var(--font-mono)',fontWeight:800,color:'var(--emerald)',fontSize:13 }}>
                            {s.fee ? fmtCLP(s.fee) : '—'}
                          </span>
                        </td>
                      </tr>
            ...
            {shifts.length > 0 && (
              <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:14,padding:'14px 16px',background:'var(--navy-50)',borderRadius:'var(--r)',border:'1px solid var(--navy-200)' }}>
                ...
                <span style={{ fontFamily:'var(--font-display)',fontWeight:800,fontSize:18,color:'var(--emerald)' }}>
